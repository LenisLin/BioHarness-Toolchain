...
Only promoted methods should receive `MethodExecutionPlanningRecord` work. For each promoted method, engineers should derive both a Layer 3 execution surface and a Layer 4 backend binding from one planning record. The Layer 3 surface aligns the agent-facing interface across methods; the Layer 4 draft records method-specific functions, wrappers, conversions, environment evidence, and failure translation. The outputs remain separate: a Layer 3 `ExecutionSurfaceSpec` and a Layer 4 `BackendAdapterSpec`.

The current generic method execution planning template is `MethodExecutionPlanningRecord v0.7.1`. It preserves the v0.7 architecture and adds stricter review rules: every required functional coverage point in the Layer 3 execution surface must have Layer 4 binding coverage, evidence resolution must be recorded, and acceptance is split across template acceptance, implementation readiness, and production readiness.
