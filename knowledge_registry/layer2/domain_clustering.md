...
| ConGI | identify spatial domains using matched image or morphology cues with expression | spatial expression, coordinates, and matched histology/image features | discrete spatial domain labels | image plus spatial-neighborhood prior | histology/image-informed | single-slice first; cross-slice support unclear | discrete spatial domain labels | expression + image/morphology branch | Optional GPU; code available |
...
If matched histology, morphology, or image signal should actively guide domain boundaries and image-aware spatial input is available:
- Prefer / consider `ConGI`, `conST`, `DeepST`, `iIMPACT`, `SiGra`, `SpaGCN`, or `SpaSEG`.
- Use image-required methods when matched image or morphology signal is part of the dataset and the scientific question.
...
- Prefer / consider `Pianno` or `SpaTopic`.
- Route spatial topic-model methods to Program Discovery when the desired output is spatial topics, topic proportions, or gene modules rather than observation-aligned domain labels.
